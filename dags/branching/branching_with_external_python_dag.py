"""
## Toy example of using the @task.branch_external_python decorator

The @task.external_python decorator is used to run any Python code in an
existing isolated Python environment to create conditional logic.
...
- [ExternalPythonOperator full list of parameters](https://registry.astronomer.io/providers/apache-airflow/versions/latest/modules/BranchExternalPythonOperator)
"""
...
@dag(
    ...
    schedule=None,
    doc_md=__doc__,
    description="@task.branch_external_python",
    default_args={
        "owner": "airflow",
        "retries": 0,
    },
    tags=["@task.branch_external_python"],
)
def branching_with_external_python_dag():
...
branching_with_external_python_dag()
